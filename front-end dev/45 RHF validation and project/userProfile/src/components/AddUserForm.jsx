...
const AddUserForm = ({ setUsers, setToggle, useredit , setUseredit, users}) => {
  const {
    register,
    handleSubmit,
    reset,
  ...
  const handleFromSubmit = (data) => {
    if (useredit) {
      setUsers((prev) =>
        prev.map((val) =>
          val.id === useredit.id ? { ...val, ...data } : val
        )
      );
      setUseredit(null);
    } else {
  ...
  return (
    <div className="h-[90%] flex flex-col gap-10 justify-center items-center mt-2">
      <h1 className="text-3xl font-bold">
        {useredit ? "Update User" : "Add a user"}
      </h1>
      ...
      >
        ...
        <button
          ...
          type="submit"
          className={`${
            !isValid ? "bg-gray-500 cursor-not-allowed" : "bg-blue-600"
          } text-white py-3 text-xl rounded-xl`}
        >
...
// const AddUserForm = ({ setUsers, setToggle , useredit}) => {
//   let {
//     register,
//     handleSubmit,
//     reset,
...
//       setUsers((prev) => {
//         return prev.map((val) => {
//           return val.id === useredit.id ? {...val , ...data} : user;
//         })
//       })
//     }else{
//      // console.log(data);
